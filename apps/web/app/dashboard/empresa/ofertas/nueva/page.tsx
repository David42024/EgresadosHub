...
const STEPS = [
  ...
  { id: 3, title: 'Finalizar', icon: CheckCircle2 },
];
...
export default function NuevaOfertaPage() {
  ...
  const createMutation = (trpc as any).ofertas.create.useMutation({
    onSuccess: (data: any) => {
      toast({
        title: "Oferta publicada",
        description: "Tu oportunidad laboral ya está disponible para postulaciones.",
      });
      router.push(`/dashboard/empresa/ofertas/${data.id}`);
    },
    onError: (error: any) => {
      toast({
        variant: "destructive",
  ...
  const nextStep = async () => {
    ...
    else toast({ variant: "destructive", title: "Faltan campos", description: "Por favor revisa los errores en el formulario." });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-1000 pb-20">
      <div className="text-center space-y-2">
        <h1 className="text-4xl font-black tracking-tight text-text-primary">Crear Convocatoria</h1>
        <p className="text-text-secondary font-medium">Completa los detalles para atraer al mejor talento universitario.</p>
      </div>
      ...
      <Card variant="elevated" className="border border-border/50 shadow-2xl bg-surface/60 backdrop-blur-md rounded-3xl overflow-hidden">
        <form onSubmit={(e) => e.preventDefault()}>
          <CardContent className="p-8 md:p-12">
            {step === 1 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-3">
                  <label className="text-xs font-bold uppercase tracking-widest text-text-muted">Título de la Oferta</label>
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-6 pt-10 border-t border-border/50">
                   <h4 className="text-lg font-bold text-text-primary flex items-center gap-2">
                    <DollarSign className="h-5 w-5 text-success" /> Rango Salarial
                  </h4>
            ...
            {step === 3 && (
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="p-8 rounded-3xl bg-bg-base/50 border border-border flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
                  <div className="h-16 w-16 rounded-2xl bg-primary-600/10 flex items-center justify-center text-primary-600">
                    <Rocket className="h-8 w-8" />
                  </div>
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold text-text-primary">Todo listo para publicar</h3>
                    <p className="text-sm text-text-secondary font-medium">Revisa los datos finales antes de confirmar la publicación de la oferta.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase tracking-widest text-text-muted">Compensación</p>
                    <p className="text-2xl font-black text-text-primary">
                      S/ {Number(watch("salarioMin")).toLocaleString()} - S/ {Number(watch("salarioMax")).toLocaleString()}
                    </p>
                    <p className="text-[10px] text-success font-black uppercase tracking-widest">Sueldo mensual bruto</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {step < 3 ? (
              ...
              <AlertDialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
                ...
                <AlertDialogContent className="rounded-3xl border-border bg-surface p-8">
                  <AlertDialogHeader>
                    <AlertDialogTitle className="text-2xl font-black">¿Confirmar Publicación?</AlertDialogTitle>
                    <AlertDialogDescription className="text-base font-medium text-text-secondary pt-2">
                      La oferta será visible inmediatamente para los egresados. Asegúrate de que los rangos salariales y requisitos sean correctos.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
